...
function readAccessFile(): Access {
  try {
    ...
    process.stderr.write(
      `discord: access.json is corrupt, moved aside. Starting fresh.\n`,
    )
  ...
  for (const senderId of files) {
    ...
    void (async () => {
      try {
        ...
        process.stderr.write(
          `discord channel: failed to send approval confirm: ${err}\n`,
        )
    ...
    for (const userId of access.allowFrom) {
      void (async () => {
        try {
          ...
          process.stderr.write(
            `permission_request send to ${userId} failed: ${e}\n`,
          )
        }
      })()
    }
    ...
    switch (req.params.name) {
      ...
      case 'download_attachment': {
        ...
        for (const att of msg.attachments.values()) {
          ...
          lines.push(
            `  ${path}  (${safeAttName(att)}, ${att.contentType ?? 'unknown'}, ${kb}KB)`,
          )
        }
        return {
          content: [
...
client.on('interactionCreate', async (interaction: Interaction) => {
  ...
  if (!access.allowFrom.includes(interaction.user.id)) {
    await interaction
      .reply({ content: 'Not authorized.', ephemeral: true })
      .catch(() => {})
    return
  }
  ...
  if (behavior === 'more') {
    ...
    await interaction
      .update({ content: expanded, components: [row] })
      .catch(() => {})
    return
  }
...
async function handleInbound(msg: Message): Promise<void> {
  ...
  if (result.action === 'pair') {
    ...
    try {
      ...
      process.stderr.write(
        `discord channel: failed to send pairing code: ${err}\n`,
      )
    }
    return
  }
  ...
  mcp
    ...
    .catch((err) => {
      process.stderr.write(
        `discord channel: failed to deliver inbound to Claude: ${err}\n`,
      )
    })
}
