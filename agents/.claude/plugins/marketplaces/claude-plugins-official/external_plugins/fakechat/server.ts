...
type Wire = ({ type: 'msg' } & Msg) | { type: 'edit'; id: string; text: string }
  ...
  tools: [
    {
      name: 'reply',
      description:
        'Send a message to the fakechat UI. Pass reply_to for quote-reply, files for attachments.',
      inputSchema: {
        type: 'object',
        properties: {
          text: { type: 'string' },
...
Bun.serve({
  ...
  websocket: {
    ...
    message: (_, raw) => {
      try {
        const { id, text } = JSON.parse(String(raw)) as {
          id: string
          text: string
        }
