...
try {
  ...
  if (stale > 1 && stale !== process.pid) {
    ...
    process.stderr.write(
      `telegram channel: replacing stale poller pid=${stale}\n`,
    )
...
bot.catch((err) => {
  process.stderr.write(
    `telegram channel: handler error (polling continues): ${err.error}\n`,
  )
})
    ...
    try {
      await bot.start({
        onStart: (info) => {
          ...
          process.stderr.write(
            `telegram channel: polling as @${info.username}\n`,
          )
          void bot.api
            .setMyCommands(
              [
      ...
      const detail = is409
        ...
        : `polling error: ${err}`
      process.stderr.write(
        `telegram channel: ${detail}, retrying in ${delay / 1000}s\n`,
      )
