...
1. **Full Disk Access** — run `ls ~/Library/Messages/chat.db`. If it fails
   with "Operation not permitted", FDA isn't granted. Say: _"Grant Full Disk
...
3. **What next** — end with a concrete next step based on state:
   ...
   - FDA granted, policy is allowlist → _"Text yourself from any device
     signed into your Apple ID — self-chat always bypasses the gate. To let
     someone else through: `/imessage:access allow +15551234567`."_
   - FDA granted, someone allowed → _"Ready. Self-chat works; {N} other
     sender(s) allowed."_
...
1. Read the allowlist. Tell the user who's in it (self-chat always works
   regardless).
2. Ask: _"Besides yourself, who should be able to text you through this?"_
...
5. **Current policy is `pairing`** → flag it immediately: _"Your policy is
   `pairing`, which auto-replies a code to every contact who texts this Mac.
   Switch back to `allowlist`?"_ and offer `/imessage:access policy
