...
const STATE_DIR =
  ...
  join(homedir(), '.claude', 'channels', 'imessage')
...
const qWatermark = db.query<{ max: number | null }, []>(
...
const qChatInfo = db.query<
  { display_name: string | null; style: number },
  [string]
>(`
...
const SELF = new Set<string>()
{
  type R = { addr: string }
  const norm = (s: string) =>
    (/^[A-Za-z]:/.test(s) ? s.slice(2) : s).toLowerCase()
...
process.stderr.write(
  `imessage channel: self-chat addresses: ${[...SELF].join(', ') || '(none)'}\n`,
)
...
function readAccessFile(): Access {
  try {
    ...
    process.stderr.write(
      `imessage: access.json is corrupt, moved aside. Starting fresh.\n`,
    )
...
function allowedChatGuids(): Set<string> {
  ...
  const handles = new Set([
    ...access.allowFrom.map((h) => h.toLowerCase()),
    ...SELF,
  ])
...
function checkApprovals(): void {
  ...
  for (const senderId of files) {
    ...
    if (err)
      process.stderr.write(
        `imessage channel: approval confirm failed: ${err}\n`,
      )
    rmSync(file, { force: true })
  }
}
...
function sendText(chatGuid: string, text: string): string | null {
  ...
  if (res.status !== 0)
    return res.stderr.trim() || `osascript exit ${res.status}`
...
mcp.setNotificationHandler(
  ...
  async ({ params }) => {
    ...
    for (const guid of targets) {
      ...
      if (err) {
        process.stderr.write(
          `imessage channel: permission_request send to ${guid} failed: ${err}\n`,
        )
      }
    }
  },
      ...
      inputSchema: {
        ...
        properties: {
          chat_guid: {
            type: 'string',
            description:
              'A specific chat_id to read. Omit to read from every allowlisted chat.',
          },
          limit: {
            type: 'number',
    ...
    switch (req.params.name) {
      case 'reply': {
        ...
        if (!allowedChatGuids().has(chat_id)) {
          throw new Error(
            `chat ${chat_id} is not allowlisted — add via /imessage:access`,
          )
        }
        ...
        for (const f of files) {
          ...
          if (st.size > MAX_ATTACHMENT_BYTES) {
            throw new Error(
              `file too large: ${f} (${(st.size / 1024 / 1024).toFixed(1)}MB, max 100MB)`,
            )
          }
        }
        ...
        for (let i = 0; i < chunks.length; i++) {
          ...
          if (err)
            throw new Error(
              `chunk ${i + 1}/${chunks.length} failed (${sent} sent ok): ${err}`,
            )
          sent++
        }
        for (const f of files) {
          ...
          if (err)
            throw new Error(
              `attachment ${basename(f)} failed (${sent} sent ok): ${err}`,
            )
          sent++
        }
        ...
        return {
          content: [
            { type: 'text', text: sent === 1 ? 'sent' : `sent ${sent} parts` },
          ],
        }
      }
      case 'chat_messages': {
        ...
        if (guid != null && !allowed.has(guid)) {
          throw new Error(
            `chat ${guid} is not allowlisted — add via /imessage:access`,
          )
        }
        if (targets.length === 0) {
          return {
            content: [
              {
                type: 'text',
                text: '(no allowlisted chats — configure via /imessage:access)',
              },
            ],
          }
        }
...
process.stderr.write(
  `imessage channel: watching chat.db (watermark=${watermark})\n`,
)
...
function handleInbound(r: Row): void {
  ...
  if (r.chat_style == null) {
    process.stderr.write(
      `imessage channel: undefined chat.style (chat: ${r.chat_guid}) — dropping\n`,
    )
    return
  }
  ...
  if (!isSelfChat) {
    ...
    if (result.action === 'pair') {
      ...
      if (err)
        process.stderr.write(
          `imessage channel: pairing code send failed: ${err}\n`,
        )
      return
    }
  }
  ...
  if (permMatch) {
    ...
    if (err)
      process.stderr.write(
        `imessage channel: permission ack send failed: ${err}\n`,
      )
    return
  }
