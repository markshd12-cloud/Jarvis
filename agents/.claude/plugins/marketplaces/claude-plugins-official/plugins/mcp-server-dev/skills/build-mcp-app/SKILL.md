...
registerAppTool(
  ...
  {
    description:
      'Open an interactive contact picker. User selects one contact.',
    annotations: { title: 'Pick Contact', readOnlyHint: true },
    inputSchema: {
      filter: z.string().optional().describe('Name/email prefix filter'),
    },
      ...
      for (const c of contacts) {
        ...
        li.addEventListener('click', () => {
          app.sendMessage({
            role: 'user',
            content: [
              { type: 'text', text: `Selected contact: ${c.id} (${c.name})` },
            ],
          })
        })
        ul.append(li)
