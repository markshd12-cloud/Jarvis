...
| **External links**   | Use `app.openLink`. Declare each origin (e.g. `https://api.example.com`) in the connector's _Allowed link URIs_ so the link skips the confirm modal.                                                                     |
